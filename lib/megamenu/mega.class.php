...
class MenuMega
{
    ...
    public function getGeneralConfig() {
        $config = array();
        $config['shownumproduct'] = Mage::getStoreConfig("bc_megamenu/bc_megamenu/shownumproduct");
        return $config;
    }
    ...
    public function getList($collections){
        $active =  Mage::helper("megamenu")->getActivemenu($collections);
    ...
    function beginMenu(){
        $effect = Mage::helper("megamenu")->get("animation");
    ...
    function genMenuItem($item, $level = 0, $pos = '', $ret = 0)
    {
        ...
        if ($tmp->image !=""){
            $image= preg_replace("/megamenu\//", "", $tmp->image);
            $txt = '<span class="menu-title" style="background-image: url('.Mage::getBaseUrl('media')."megamenu/".$image.');">' . $tmpname . '</span>';
        }else {
